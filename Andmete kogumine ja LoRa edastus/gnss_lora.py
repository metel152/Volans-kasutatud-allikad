...
try:
    ...
    configure_galileo_only(gnss)
except Exception as e:
    gnss = None
    print("GNSS avamise viga:", e)
...
try:
    while True:
        ...
        try:
            ...
            gnss_lat_s = f'{gnss_data["lat_decimal"]:.8f}' if gnss_data is not None else "nan"
            gnss_lon_s = f'{gnss_data["lon_decimal"]:.8f}' if gnss_data is not None else "nan"
            gnss_alt_s = f'{gnss_data["alt_m"]:.2f}' if (gnss_data is not None and gnss_data["alt_m"] is not None) else "nan"
            gnss_sats_s = str(gnss_data["sats"]) if (gnss_data is not None and gnss_data["sats"] is not None) else "nan"

            telemetry_packet = (
                f"TEL,{telemetry_seq},{now},"
                f"{temp_s},{rohk_s},{alt_s},"
                f"{ax_s},{ay_s},{az_s},{gx_s},{gy_s},{gz_s},"
                f"{gnss_lat_s},{gnss_lon_s},{gnss_alt_s},{gnss_sats_s}"
            )
