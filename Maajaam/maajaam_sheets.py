...
class GoogleSheetsUploader:
    ...
    def start(self):
        if not _HAS_GSPREAD:
            self.log("gspread / google-auth not installed – Sheets upload disabled.")
            return
    ...
    def _run(self):
        ...
        while not self._stop_event.is_set():
            self._notify_event.wait(timeout=5)   # wake on notify() or every 5 s
            self._notify_event.clear()
            try:
                self._flush_telemetry()
                self._flush_gps()
            except Exception as exc:
                self.log(f"Sheets upload error: {exc}")
    ...
    def _connect(self):
        ...
        # Count rows already in the sheets so we don't duplicate on restart
        self._tel_sent = max(0, len(self._ws_tel.get_all_values()) - 1)
        self._gps_sent = max(0, len(self._ws_gps.get_all_values()) - 1)

        self.log(f"Connected to Google Sheet. "
                 f"Resuming from tel row {self._tel_sent}, "
                 f"gps row {self._gps_sent}.")

        # Add charts once (silently skip if it fails – needs Sheets API v4)
        try:
    ...
    def _flush_telemetry(self):
        self._flush_csv(self.telemetry_csv, self._ws_tel,
                        "_tel_sent", self._TELEMETRY_HEADERS)

    def _flush_gps(self):
        self._flush_csv(self.gps_csv, self._ws_gps,
                        "_gps_sent", self._GPS_HEADERS)

    def _flush_csv(self, csv_path: str, ws, sent_attr: str, expected_headers):
        ...
        setattr(self, sent_attr, already_sent + len(new_rows))
        self.log(f"Uploaded {len(new_rows)} new row(s) → '{ws.title}'.")
    ...
    def _add_telemetry_charts(self):
        ...
        Adds three line charts to the Telemetry sheet:
          • Temperature (col C) vs Timestamp (col B)
          • Pressure    (col D) vs Timestamp (col B)
          • Altitude    (col E) vs Timestamp (col B)
        """
        sheet_id = self._ws_tel.id
        spreadsheet_id = self._spreadsheet.id

        # Column indices (0-based): B=1 timestamp, C=2 temp, D=3 pressure, E=4 alt
        chart_specs = [
            ("Temperature (°C)",   2, 0,   0),   # (title, data_col, anchor_col, anchor_row)
            ("Pressure (hPa)",     3, 7,   0),
            ("Altitude (m)",       4, 14,  0),
        ]
...
class SheetsConfigDialog(tk.Toplevel):
    ...
    def __init__(self, parent):
        ...
        self.result = None   # will be set to (url, creds_path) on OK
...
class GroundStationApp:
    def __init__(self, root):
        ...
        self.gps_csv = os.path.join(self.log_dir, "gps.csv")
    ...
    def handle_gps(self, line):
        ...
        new_file = not os.path.exists(self.gps_csv)
        with open(self.gps_csv, "a", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            if new_file:
                w.writerow([
                    "seq", "timestamp", "latitude", "longitude",
            ...
            w.writerow([seq, timestamp, lat, lon, gnss_alt, sats, fix])
    ...
    def toggle_sheets_upload(self):
        ...
        sheet_url, creds_path = dlg.result
